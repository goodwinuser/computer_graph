...
static void MakeScene(Scene* s) {
	...
	auto t = Entity(&s->meshes[0], &s->shaders[0]);
	t.scale = glm::vec3(4.0f, 4.0f, 4.0f);
	s->entities.push_back(t);

	auto cup1 = Entity(&s->meshes[1], &s->shaders[0]);
	cup1.position += glm::vec3(0.5f, 0.0f, 0.0f);
	cup1.scale = glm::vec3(0.3f, 0.3f, 0.3f);
	s->entities.push_back(cup1);

	auto cup2 = Entity(&s->meshes[2], &s->shaders[0]);
	cup2.position += glm::vec3(-2.5f, 0.0f, 0.0f);
	cup2.scale = glm::vec3(0.1f, 0.1f, 0.1f);
	s->entities.push_back(cup2);
}

int main() {
	sf::Window window(sf::VideoMode(SCREEN_WIDTH, SCREEN_HEIGHT), "Sun system", sf::Style::Default, sf::ContextSettings(24));
	SetIcon(window);
